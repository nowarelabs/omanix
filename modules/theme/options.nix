# modules/theme/options.nix — theme, tiling, and per-app overrides (the green user surface)
# Extends core omanix.theme enum with per-theme overrides, Omatiles (bridge onto macOS's
# own Sequoia window tiling), and per-app/per-color overrides. Both build on the OS
# instead of replacing it — no yabai, no sketchybar, no external bar daemons.

...
macOS cannot run Hyprland / SDDM / Wayland natively. Hard line: **we do not hide a Linux VM**. (Approved: bar via the external `spacebar` daemon configured from Nix, tiling via the native Omatiles module — no AeroSpace binary, no SketchyBar process.)
...
- **Quickshell → spacebar:** Quickshell's bar is served by the external **spacebar** daemon (`modules/darwin/spacebar.nix` → nix-darwin `services.spacebar`), fully configured from `omanix.spacebar.*` Nix options with theme colors injected at build time. Not a port of a shell config — a real bar binary, themed from the flake.
...
- **Spacebar/Omatiles:** The user sets `omanix.spacebar.showClock = true` or `omanix.omatiles.enableMargins = true` as Nix options. They never edit a bar/tiling config by hand; the bar reads the options via `services.spacebar`, the app at runtime.
...
- **No custom packaging:** Do not create `packages/foo.nix` if `nixpkgs` or `homebrew` already ships it. `packages/` is only for Omanix-specific adapters (e.g., `omarchy-nvim` wrapper, the Omanix app itself with its Omatiles module). Before adding to `packages/`, you must show `nix search` and `brew search` miss.
...
- **UX:** Either way, it may produce any of: a `launchd` agent (`launchd.user.agents.pomodoro`), a **Swift/SwiftUI app** (`packages/pomodoro-app` → `/Applications` via `home.file`), or — for bar items — options under `omanix.spacebar.*` feeding the spacebar daemon. All hashed and added via options.
...
- No `~/.config/hypr/*`, `~/.config/sketchybar/*`, `~/.config/aerospace/*` exists (or survives) as a hand-edited file — those stacks were removed; the bar is the spacebar daemon configured from Nix, tiling lives inside the Omanix app. What remains is store-symlinked and backed up via `home-manager.backupFileExtension = "backup"`, so uninstall leaves no Omanix residue.
...
Your friend's map is right on the end goal — _user says "make pomodoro", Claude writes it, it just appears themed and isolated_ — but wrong on the macOS substrate if taken literally. Friend assumes NixOS + `~/.config` impure Home Manager imports + Quickshell QML hot-reload via D-Bus. Omanix is Darwin-native: **bar via the external spacebar daemon (configured from Nix) + native Omatiles SwiftUI modules in the Omanix app** + `launchd` + `Swift` on macOS, `Hyprland`/`Quickshell`/`systemd` on future Linux. Hard line: **Omanix is AI-ready by providing a typed Nix contract, a machine-readable skill, and a two-phase build (preview → commit) that is mac-native and cross-platform, not by letting an agent `echo` into `/nix/store` or `~/.config` imperatively.**
...
| Quickshell `Qt.createComponent()` + D-Bus IPC to reload                                                                     | **Mac:** `launchctl kickstart -k gui/$UID/om.omanix.omatiles` + `launchctl load ~/Library/LaunchAgents/org.omanix.*.plist` (generated) + spacebar re-configured by Nix on rebuild. **Future Linux:** `quickshell ipc call` + `systemctl --user daemon-reload`. Same `omanix rebuild` triggers the right IPC branch via `pkgs.stdenv.isDarwin`.                                                                                                                                                                    |
...
3. **Agent (preview, impure, instant, no generation):** `omanix rebuild --preview` — Nix evaluates `overlays/*` impurely (`builtins.readDir ../overlays`), builds `/nix/store/...-pomodoro` with `${theme.colors.accent}` injected, hot-reloads via `launchctl kickstart -k gui/$UID/om.omanix.omatiles` (mac) / `quickshell ipc` + `systemctl` (linux). Widget appears instantly.
...
│   │   ├── nix.nix
│   │   └── fonts.nix
│   ├── darwin/                  # mac-only: homebrew (pristine), pam Touch ID, system.defaults, spacebar, omatiles
│   │   ├── homebrew.nix
│   │   ├── spacebar.nix         # menu bar via the `spacebar` daemon (nix-darwin services.spacebar, theme colors injected)
...
│   │   ├── hyprland.nix
│   │   └── quickshell.nix
│   ├── desktop/                 # picks at eval: if isDarwin then darwin/spacebar.nix + omatiles.nix else linux/hyprland.nix — you just set omanix.theme
│   ├── theme/                   # themed.nix: themes/*/colors.toml + default/themed/*.tpl → /nix/store — you never touch tpl
│   ├── widgets/                 # omanix.widgets.* — pomodoro etc.
│   │   ├── pomodoro.nix         # uses lib/mkWidget: launchd (mac) / systemd (linux) + optional Swift/GTK app
│   │   └── clock.nix

...
> **Build-time purity.** You set `omanix.theme = "tokyo-night"` — Nix renders `Ghostty` colors and `Omatiles` (window tiling) from the palette. The menu bar is the native macOS menu bar. You never edit `colors.toml` or `.tpl` by hand. See `principles.md:3`.
...
```
Then: `omanix rebuild` — `Ghostty` + `Omatiles` reload with new options.
...
## Window Tiling Options (Omatiles)

The menu bar is the **native macOS menu bar** (the old bar daemon and the Omabar plugin-host code are archived in `ice/`). Tiling is the native **Omatiles** module (`omanix.omatiles.*`) that bridges onto macOS Sequoia's built-in tiling.
...
default/themed/*.tpl (future)─┘         ├─► ghostty/config (xdg.configFile."ghostty/config")
                                         ├─► ~/.config/omanix/theme.json (runtime palette for Omatiles + Store)
...
ice/spacebar/spacebar.nix — archived; the menu bar is the native macOS menu bar (no bar daemon)
...
- `ice/spacebar/spacebar.nix` — the archived spacebar daemon module (nix-darwin `services.spacebar` launchd agent; theme colors mapped to `0xffRRGGBB`; bar icon font `pkgs.font-awesome`). The menu bar is the native macOS menu bar.
...
**Community submission:** PR with `themes/<name>/colors.toml` + preview + `vscode.json` + enum entry. Do not hand-edit `lib/themed.nix` — new colors automatically propagate to Ghostty/Omatiles/widgets.
...
Global `omanix.themeOverrides` affects all apps. For per-app divergence (e.g. keep Ghostty on pure black but Omatiles/widgets on the theme palette):
...
```nix
{
  ...
  # Omatiles + Widgets read the global palette (theme.json); per-app terminal divergence only.
}
# CLI:
# omanix theme per-app ghostty background "#000000"
```

Resolution: `base = themes/<name>/colors.toml` → `// omanix.themeOverrides` → `// omanix.perApp.<app>` (via `lib/themed.nix:getAppColors`, spec alias `theme.perApp` also read if set). Ghostty uses `ghostty` delta; Omatiles/widgets use global (`theme.json`).
...
Implementation: `modules/theme/theme.nix` writes `~/.config/omanix/theme.json`; the menu bar is the native macOS menu bar (the old bar daemon is archived in `ice/`), and the macOS tiling preferences are set declaratively by the activation scripts — no `sketchybar --reload` and no custom geometry. Disable (`enable = false`) for instant cut (useful for screenshots/tests). Nix path is `omanix.transition` because `omanix.theme` is a string enum (cannot nest); spec alias `theme.transition` also honoured. All are `<30s` rollbackable via generations.

---

## Store Integration

`Super → Omanix Store → Themes` (12 cards) reads `vm.themes` (hardcoded from `themes/*/colors.toml`, bridged via `OColor(hex:)`) and `store.currentThemeId()` (`~/.config/omanix/theme.json` → `configuration.nix`). Tap **Select** → `store.setTheme(id)` → `needsRebuild` banner → **Rebuild**. Tiling (`layout`/`gaps`/`bindings`/`watch`/`floatingApps`) is `omanix.omatiles.*`, applied live to the running module. The menu bar is the native macOS menu bar. See `modules/apps/gui/Views/ThemesView.swift:27` and `ViewModels/OmanixViewModel.swift:228`.
...
4. **Preview:** `omanix theme set <my-theme> && omanix rebuild` (impure preview via `--preview` re-themes the running GUI from `theme.json`). Add `backgrounds/*.jpg` and `preview.png` (16:9, 1200x675).
...
- Colors not updating — Omatiles/the Store re-read `~/.config/omanix/theme.json` on start and on live apply; launch again if a module was stopped.

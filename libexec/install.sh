...
install_clone() {
  ...
  # version is machine-owned + gitignored — write it at install time. state.nix is
  # tracked (fresh clone already has the template), so ensure is a no-op safety net.
  git -C "$FLAKE_DIR" rev-parse --short HEAD > "$FLAKE_DIR/version" 2>/dev/null || true
  "$FLAKE_DIR/libexec/omanix-state.sh" ensure >/dev/null 2>&1 || true
